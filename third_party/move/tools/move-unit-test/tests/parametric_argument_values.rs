...
#[test]
fn string_try_utf8_valid_converts_to_some_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::string;
            #[test(s = string::try_utf8(vector[104, 105]))]
            fun test_try_utf8(s: std::option::Option<std::string::String>) {
                let _ = s;
            }
        }
        }
    ...
    assert_eq!(module.tests.get("test_try_utf8").unwrap().arguments, vec![
        MoveValue::Struct(MoveStruct::new(vec![MoveValue::Vector(vec![
            MoveValue::Struct(MoveStruct::new(vec![MoveValue::Vector(vec![
                MoveValue::U8(104),
                MoveValue::U8(105),
...
#[test]
fn string_try_utf8_invalid_converts_to_none_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::string;
...
const SELF_CONTAINED_ENUM_OPTION_MODULE: &str = r#"
    module option {
...
#[test]
fn option_from_vec_empty_converts_to_none_variant_layout() {
    let source = format!(
        r#"
        address 0x1 {{
        {}
        module M {{
            ...
            fun test_from_vec(o: Option<u8>) {{
                let _ = o;
            }}
        }}
        }}
    "#,
        SELF_CONTAINED_ENUM_OPTION_MODULE
    );

    let plan = build_test_plan_from_source_no_stdlib(&source).unwrap();
    let module = plan.module_tests.values().next().unwrap();

    assert_eq!(module.tests.get("test_from_vec").unwrap().arguments, vec![
        MoveValue::Struct(MoveStruct::new_variant(0, vec![]))
    ]);
}

#[test]
fn option_from_vec_single_element_converts_to_some_variant_layout() {
    let source = format!(
        r#"
        address 0x1 {{
        {}
    ...
    assert_eq!(module.tests.get("test_from_vec").unwrap().arguments, vec![
        MoveValue::Struct(MoveStruct::new_variant(1, vec![MoveValue::U8(5)]))
    ]);
}

#[test]
fn ascii_string_valid_converts_to_byte_vector_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii::{Self, String};
            #[test(s = ascii::string(vector[104, 105]))]
            fun test_ascii(s: String) {
                let _ = s;
            }
        }
        }
    ...
    assert_eq!(module.tests.get("test_ascii").unwrap().arguments, vec![
        MoveValue::Struct(MoveStruct::new(vec![MoveValue::Vector(vec![
            MoveValue::U8(104),
            MoveValue::U8(105),
        ])]))
...
#[test]
fn ascii_string_invalid_byte_conversion_is_rejected() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii::{Self, String};
    ...
    // `report_conversion_error` only records a diagnostic; nothing later checks
    // `env.has_errors()`, so `build_test_plan` still succeeds. The rejected argument surfaces as
    // missing from `arguments` rather than as a build failure.
    assert!(module.tests.get("test_ascii").unwrap().arguments.is_empty());
}

#[test]
fn ascii_try_string_valid_converts_to_some_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii;
            #[test(s = ascii::try_string(vector[104, 105]))]
            fun test_try_string(s: std::option::Option<ascii::String>) {
                let _ = s;
            }
        }
        }
    ...
    assert_eq!(
        module.tests.get("test_try_string").unwrap().arguments,
        vec![MoveValue::Struct(MoveStruct::new(vec![MoveValue::Vector(
            vec![MoveValue::Struct(MoveStruct::new(vec![MoveValue::Vector(
                vec![MoveValue::U8(104), MoveValue::U8(105),]
            )]))]
        )]))]
    );
}
...
#[test]
fn ascii_try_string_invalid_converts_to_none_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii;
...
#[test]
fn ascii_char_valid_converts_to_u8_field_layout() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii::{Self, Char};
            #[test(c = ascii::char(104))]
            fun test_char(c: Char) {
                let _ = c;
            }
        }
        }
    ...
    assert_eq!(module.tests.get("test_char").unwrap().arguments, vec![
        MoveValue::Struct(MoveStruct::new(vec![MoveValue::U8(104)]))
    ]);
}

#[test]
fn ascii_char_invalid_byte_conversion_is_rejected() {
    let source = r#"
        address 0x1 {
        module M {
            use std::ascii::{Self, Char};
    ...
    assert!(module.tests.get("test_char").unwrap().arguments.is_empty());
}

#[test]
fn byte_string_literal_converts_to_vector_u8_layout() {
    let source = r#"
        address 0x1 {
        module M {
            #[test(xs = b"hi")]
            fun test_bytes(xs: vector<u8>) {
                let _ = xs;
            }
        }
        }
    ...
    assert_eq!(module.tests.get("test_bytes").unwrap().arguments, vec![
        MoveValue::Vector(vec![MoveValue::U8(104), MoveValue::U8(105)])
    ]);
}

#[test]
fn hex_string_literal_converts_to_vector_u8_layout() {
    let source = r#"
        address 0x1 {
        module M {
            #[test(xs = x"6869")]
            fun test_hex(xs: vector<u8>) {
                let _ = xs;
            }
        }
        }
    ...
    assert_eq!(module.tests.get("test_hex").unwrap().arguments, vec![
        MoveValue::Vector(vec![MoveValue::U8(104), MoveValue::U8(105)])
    ]);
}

#[test]
fn option_from_vec_too_long_conversion_is_rejected() {
    let source = format!(
        r#"
        address 0x1 {{
        {}
    ...
    assert!(module.tests.get("too_long").unwrap().arguments.is_empty());
}
